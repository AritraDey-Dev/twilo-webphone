...
import { api } from './api';
import { createDevice } from './phone';
...
import { IconKeypad, IconMessage, IconPhone, IconSupport, IconLogout } from './components/icons.jsx';
...
const STATUS_LABEL = { ready: 'Live', offline: 'Offline', connecting: 'Connecting', error: 'Error' };
...
export default function App() {
  ...
  useEffect(() => {
    api.me().then((m) => { setMe(m); setAuthed(true); }).catch(() => setAuthed(false));
  }, []);
  ...
  useEffect(() => {
    ...
    createDevice({
      ...
      onIncoming: (call) => {
        ...
        call.on('disconnect', () => { setIncoming(null); setActiveCall(null); });
        call.on('reject', () => setIncoming(null));
      },
    })
      .then((d) => { device = d; deviceRef.current = d; })
  ...
  const hangup = () => { if (activeCall) activeCall.disconnect(); setActiveCall(null); };
  const acceptIncoming = () => { if (!incoming) return; incoming.accept(); setActiveCall(incoming); setIncoming(null); };
  const rejectIncoming = () => { if (incoming) incoming.reject(); setIncoming(null); };
  ...
  return (
    <>
      ...
      <div className="app">
        <aside className="sidebar">
          ...
          <nav className="sb-nav">
            {TABS.map(([t, label, Icon]) => (
              <button key={t} className={`sb-item ${tab === t ? 'active' : ''}`} onClick={() => setTab(t)}>
                <Icon />
                <span>{label}</span>
              </button>
            ))}
          ...
          <div className="sb-foot">
            <div className="sb-status">
              <span className={`beacon ${status}`} />
              <span className="s-label">{STATUS_LABEL[status] || status}</span>
              <span className="s-num">{me?.number}</span>
            </div>
            <button className="sb-action"><IconSupport /><span>Need support</span></button>
            <button className="sb-action danger" onClick={logout}><IconLogout /><span>Logout</span></button>
          </div>
        </aside>
        ...
        <main className="main">
          ...
          {tab === 'dialer' && (
            <div className="card dialer">
              <Dialer onCall={startCall} activeCall={activeCall} onHangup={hangup} status={status} />
            </div>
          )}
          {tab === 'sms' && <SmsInbox />}
        ...
        {incoming && <IncomingCall call={incoming} onAccept={acceptIncoming} onReject={rejectIncoming} />}
      </div>
    </>
  );
}
